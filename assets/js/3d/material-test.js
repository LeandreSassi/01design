...
// Fresh grassOptions object per volume — must not be shared, since each
// volume's GUI mutates its own copy independently.
...
// Wraps any already-materialed THREE.Mesh (a primitive in Simple mode, a
// real production project shape in Alpha mode — see buildAlphaVolumes) into
// the same volume shape the rest of the app operates on, so selection,
// material-switching, grass, water and presets all work identically
// regardless of where the mesh came from.
function wrapAsVolume(viewport, name, mesh, materialTypeName = 'MeshStandardMaterial') {
    ...
    return { name, mesh, marker, materialTypeName, grass: null, grassOptions: defaultGrassOptions() };
}

// Tears down everything wrapAsVolume added. `disposeGeometry` should be
// false for Simple mode's shared, reused primitive geometries (module-level
// consts) and true for Alpha mode's geometries (freshly built per project
// each time real data loads — nothing else references them).
function disposeVolumeVisuals(viewport, volume, disposeGeometry) {
    viewport.scene.remove(volume.mesh, volume.marker);
    if (volume.grass) {
        volume.mesh.remove(volume.grass);
        volume.grass.geometry.dispose();
        volume.grass.material.dispose();
    ...
    volume.marker.material.dispose();
    volume.marker.geometry.dispose();
    if (disposeGeometry) volume.mesh.geometry.dispose();
}

function buildSimpleVolumes(viewport) {
    return VOLUMES.map(v => {
        ...
        return wrapAsVolume(viewport, v.name, mesh);
    });
}

// Alpha mode: the exact real matrix, built with the production code
// (Taxonomy/ProjectRepository/MatrixLayout/LaneAssignment/ProjectObjectFactory)
// against the same taxonomy.json/projects.json the live site uses — same
// geometry (including the fused multi-cell "massing" shapes, not just plain
// boxes), same per-project color. Only the material gets swapped out
// afterward, by wrapAsVolume/the rest of this app.
async function buildAlphaVolumes(viewport) {
    ...
    const volumes = allEntries.map(entry => {
        ...
        return wrapAsVolume(viewport, entry.project.title, mesh);
    });

    const radius = Math.max(...layout.extent(MATRIX.border)) / 2;
    return { volumes, radius };
}

function fitCameraToRadius(viewport, radius) {
    const distance = Math.max(radius * 2.4, 4);
    viewport.camera.position.set(distance * 0.55, distance * 0.42, distance * 0.72);
...
// --- Presets: save/load a volume's current material as a named, JSON-safe
// snapshot (localStorage-persisted, plus export/import to a real .json file
// so presets can travel with a project). One capture/apply pair per
// material type, since each has a different set of tunable fields; grass
// additionally carries volume.grassOptions rather than material properties.
...
const PRESET_HANDLERS = {
    ...
    // Grass's own tunables live on volume.grassOptions, not the material —
    // captureVolumePreset()/applyVolumePreset() below fold that bag in.
    FurGrassMaterial: {
...
function captureVolumePreset(volume) {
    const type = volume.materialTypeName;
    const handler = PRESET_HANDLERS[type] || PRESET_HANDLERS.MeshStandardMaterial;
    const data = handler.capture(volume.mesh.material);
    if (type === 'FurGrassMaterial') data.grassOptions = { ...volume.grassOptions };
    return { type, data };
}

function applyVolumePreset(volume, preset) {
    const old = volume.mesh.material;
    volume.materialTypeName = preset.type;
    const seedColor = preset.data.color ? new THREE.Color(preset.data.color) : getMaterialColor(old).clone();
    volume.mesh.material = MATERIAL_TYPES[preset.type](seedColor);
    old.dispose();

    const handler = PRESET_HANDLERS[preset.type] || PRESET_HANDLERS.MeshStandardMaterial;
    handler.apply(volume.mesh.material, preset.data);

    if (preset.type === 'FurGrassMaterial' && preset.data.grassOptions) {
        Object.assign(volume.grassOptions, preset.data.grassOptions);
    }
    updateGrass(volume);
}

const PRESETS_STORAGE_KEY = 'materialTesterPresets';
...
function savePresetsToStorage(presets) {
    localStorage.setItem(PRESETS_STORAGE_KEY, JSON.stringify(presets, null, 2));
}
...
function main() {
    ...
    // Ground is a real Water surface (three/addons/objects/Water.js) rather
    // than an opaque slab — it mirrors the volumes and sun instead of
    // shadowing them. Kept deliberately still: a low distortionScale plus a
    // slow-crawling time uniform (see the onFrame hook below) reads as calm
    // water, not an ocean. Its custom shader doesn't sample the shadow map,
    // so unlike the old slab it won't show cast shadows.
    const water = new Water(new THREE.PlaneGeometry(500, 500), {
        textureWidth: 512,
        textureHeight: 512,
        waterNormals: createWaterNormalTexture(512),
        sunDirection: sun.position.clone().normalize(),
        sunColor: 0xffffff,
        waterColor: 0xdfe6e2, // pale cool tint, close to BROKEN_WHITE so it reads as a bright still pool
        distortionScale: 0.4,
        fog: viewport.scene.fog !== undefined
    });
    water.rotation.x = -Math.PI / 2;
    water.position.y = -0.6; // meets the lowest-sitting volumes, same as the old ground's top face
    viewport.add(water);

    viewport.camera.position.set(0, 2.5, 6);

    let volumes = buildSimpleVolumes(viewport);
    ...
    // --- Simple/Alpha mode ---
    // Simple: the abstract primitives above (fast, no network/data dependency).
    // Alpha: the exact real production matrix, built from taxonomy.json/
    // projects.json with the actual matrix-building code (see
    // buildAlphaVolumes) — for confirming a material reads right on genuine
    // project geometry, not a stand-in shape.
    let currentMode = 'Simple';
    let modeLoading = false;
    async function switchMode(mode) {
        if (mode === currentMode || modeLoading) return;
        modeLoading = true;
        try {
            const wasAlpha = currentMode === 'Alpha';
            deselect();
            volumes.forEach(v => disposeVolumeVisuals(viewport, v, wasAlpha));

            if (mode === 'Alpha') {
                const built = await buildAlphaVolumes(viewport);
                volumes = built.volumes;
                fitCameraToRadius(viewport, built.radius);
            } else {
                volumes = buildSimpleVolumes(viewport);
                fitCameraToRadius(viewport, 3);
            }
            currentMode = mode;
            if (volumes.length) select(volumes[0]);
        } finally {
            modeLoading = false;
        }
    }
    guiMode(gui, switchMode);
    ...
    guiPresets(gui, {
        ...
        save: name => {
            if (!selected) return;
            presets[name] = captureVolumePreset(selected);
            savePresetsToStorage(presets);
        },
        load: name => {
            if (!selected || !presets[name]) return;
            applyVolumePreset(selected, presets[name]);
            typeProxy.type = selected.materialTypeName;
            typeController.updateDisplay();
            rebuildMaterialFolders(selected);
        },
        remove: name => {
        ...
        exportJSON: () => {
            ...
            a.href = url;
            a.download = 'material-presets.json';
            a.click();
        ...
        importJSON: text => {
            try {
                ...
                console.error('Invalid presets JSON', e);
            }
        }
    });
    ...
    viewport.onFrame(() => {
        ...
        // Crawl slowly, not at real-time speed — with distortionScale this
        // low a full-speed time uniform still looks like flowing water; this
        // is what actually makes the ground read as still.
        water.material.uniforms.time.value += dt * 0.05;
...
// --- "Presets" folder: save/load/delete/export/import named material
// snapshots for the currently selected volume. `controller` supplies the
// actual storage logic (see main()) — this just wires up the panel.
function guiPresets(gui, controller) {
    const folder = gui.addFolder('Presets');
    const data = { name: 'my-preset', preset: '' };

    folder.add(data, 'name').name('name');
    const presetDropdown = folder.add(data, 'preset', controller.list()).name('preset');

    function refreshDropdown() {
        const names = controller.list();
        presetDropdown.options(names);
        if (names.length && !names.includes(data.preset)) {
            data.preset = names[0];
        }
    }
    ...
    folder.add({ save: () => {
        ...
        refreshDropdown();
    } }, 'save').name('Save current as name');

    folder.add({ load: () => { if (data.preset) controller.load(data.preset); } }, 'load')
        .name('Load selected preset');

    folder.add({ remove: () => {
        if (!data.preset) return;
        controller.remove(data.preset);
        refreshDropdown();
    } }, 'remove').name('Delete selected preset');

    folder.add({ export: () => controller.exportJSON() }, 'export').name('Export JSON file');
    ...
    fileInput.addEventListener('change', () => {
        ...
        reader.onload = () => {
            controller.importJSON(reader.result);
            refreshDropdown();
        };
        reader.readAsText(file);
        fileInput.value = '';
    });
    folder.add({ import: () => fileInput.click() }, 'import').name('Import JSON file');

    return folder;
}

// --- "Mode" folder: Simple (abstract primitives) vs Alpha (the real
// production matrix, loaded live from taxonomy.json/projects.json). See
// switchMode() in main().
function guiMode(gui, switchMode) {
    const folder = gui.addFolder('Mode');
    const data = { mode: 'Simple' };
    folder.add(data, 'mode', ['Simple', 'Alpha']).onChange(switchMode);
    return folder;
}
...
function guiFurGrassMaterial(gui, material, volume) {
    ...
    grassFolder.add(opts, 'density', 200, 100000, 100).onChange(() => updateGrass(volume));
